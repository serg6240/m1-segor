...
class Amasty_Fpccrawler_Helper_Data extends Mage_Core_Helper_Abstract
{
    ...
    public function generateQueue(){
        ...
        if (!Mage::getStoreConfig('amfpccrawler/general/enabled')) {
            throw new Exception('FpcCrawler is not enabled');
        }
        ...
        if (!flock($fp, LOCK_EX | LOCK_NB)) {
            throw new Exception('Another lock detected');// another lock detected
        }
        ...
        if ($source){
            ...
            throw new Exception('Crawler source is not defined');
        }
