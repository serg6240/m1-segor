...
class Amasty_Fpc_Model_Fpc_Front extends Varien_Object
{
    ...
    public function getUrlTag($url = false)
    {
        if ($url) {
            $urlComponents = parse_url($url);

            if (!isset($urlComponents['host']) || !isset($urlComponents['path']))
                throw new Exception(sprintf('Malformed url "%s"', $url));

            $urlString = $urlComponents['host'] . $urlComponents['path'];
        }
        else {
            $urlString = $_SERVER['HTTP_HOST'] . strtok($_SERVER['REQUEST_URI'], '?');
        }

        return 'url_' . sha1($urlString);
    }
    ...
    public function processIfModifiedSince($cacheTimestamp)
    {
        $request = new Zend_Controller_Request_Http();

        // "If-Modified-Since" headers can be received only from bots
        // because common visitors always receive no-cache headers from server
        if ($modifiedSince = $request->getHeader('If-Modified-Since')) {

            $modifiedSinceTimestamp = strtotime($modifiedSince);

            // Cache for current page was created before the latest bot's visit so nothing changed
            if ($cacheTimestamp <= $modifiedSinceTimestamp) {
                header_remove(); // Clear default no-cache headers
    ...
    protected function fetch()
    {
        ...
        if ($data = $this->getCache()->loadPage($key))
        {
            ...
            $this->processIfModifiedSince($data['timestamp']);
            ...
            if ($this->isDynamicBlocksEnabled())
            {
                ...
                foreach ($validBlocks as $name => $content)
                {
                    ...
                    $page = preg_replace("/<amfpc name=[\"']" . $name . "[\"'] \/>/", $content, $page);
                }
...
AJAX;
    ...
    public function extractContent()
    {
        ...
        if (isset(${'_GET'}['amfpc_refresh'])) {
            $this->getCache()->clean($this->getUrlTag());
        ...
        if (!$this->ignore())
        {
            ...
            if ($content)
            {
                ...
                if ($fetchResult['content_type']) {
                    $response->setHeader('content-type', $fetchResult['content_type']);
                }

                $response->appendBody($content)->sendResponse();
