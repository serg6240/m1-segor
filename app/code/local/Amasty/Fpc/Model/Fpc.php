...
class Amasty_Fpc_Model_Fpc extends Mage_Core_Model_Cache
{
    ...
    public function getProductsAdditionalTags($productIds)
    {
        ...
        if (!empty($productIds))
        {
            ...
            $additionalProductsBundle = Mage::getModel('bundle/product_type')->getParentIdsByChild($productIds);
            $additionalProducts = array_merge($additionalProducts, $additionalProductsBundle);
    ...
    public function clearByUrl($url)
    {
        $tag = Mage::getSingleton('amfpc/fpc_front')->getUrlTag($url);
